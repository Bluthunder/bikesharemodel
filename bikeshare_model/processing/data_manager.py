...
def pre_pipeline_preparation(*, df: pd.DataFrame) -> pd.DataFrame:
   ...
   print("Columns before extraction:", df.columns.tolist())

   df = extract_yr_mnth(df, date_column='dteday')

   print("Columns after extraction:", df.columns.tolist())

   df.drop(labels=config.model_config_.unused_features, axis=1, inplace=True)
